...
class ProteinMPNN(nn.Module):
    ...
    def sample(self, feature_dict, mode="ar", sampler=None, trajectory_writer=None):
        if mode == "ar":
            return self.sample_ar(feature_dict)
        elif mode == "dfm":
            return self.sample_dfm(feature_dict, sampler=sampler, trajectory_writer=trajectory_writer)
        else:
            raise ValueError(f"Invalid sample mode: {mode}")

    def sample_dfm(self, feature_dict, sampler=None, eps=1e-3, trajectory_writer=None):
        from samplers import EulerSampler, EDSSampler
        if sampler is None:
            sampler = EulerSampler(dt=feature_dict.get("dfm_dt", 0.1))
        is_eds = isinstance(sampler, EDSSampler)
        if is_eds:
            sampler.reset()

        #DFM analogue of sample_ar: iterates in time t in [0,1] instead of along a decoding order.
        B_decoder = feature_dict["batch_size"]
        S_true = feature_dict["S"] #[B,L]
        mask = feature_dict["mask"] #[B,L]
        chain_mask = feature_dict["chain_mask"] #[B,L] 0=fixed, 1=designable
        bias = feature_dict["bias"] #[B,L,self.num_letters]
        ...
        #repeat for decoding (mirrors sample_ar)
        ...
        feat = {k: v for k, v in feature_dict.items()}
        for key in ["X", "X_m", "R_idx", "chain_labels", "protein_mask",
                    "dna_mask", "rna_mask", "R_polymer_type"]:
            if key in feat and torch.is_tensor(feat[key]) and feat[key].shape[0] == 1:
                feat[key] = feat[key].repeat(B_decoder, *([1]*(feat[key].dim()-1)))
        feat["mask"] = mask
        #-----
        ...
        #initialise S at MAS for designable positions, S_true for fixed positions
        mask_token_id = self.restype_to_int["MAS"]
        S = torch.where(chain_mask.bool(),
                        torch.full_like(S_true, mask_token_id),
                        S_true).long()
        ...
        mask_token_id = self.restype_to_int["MAS"]
        with torch.no_grad():
            # Initial trajectory snapshot at t=0 (all-MAS over designable positions).
            if trajectory_writer is not None:
                trajectory_writer.write_step(step_idx=step_idx, t=0.0, S=S, mask_token_id=mask_token_id, probs=None)

            while True:
                if is_eds:
                    if sampler.done:
                        break
                    # Drive t from the EDS grid so successive steps land on grid edges exactly.
                    t_value = sampler.t_grid[sampler._k]
                else:
                    if not (t_value < 1.0 - eps):
                        break

                log_probs, probs_sample, p_next, h = sampler.step(
                    self, feat, S, t_value, bias, temperature, eps
                )

                S_next = torch.distributions.Categorical(probs=p_next).sample() #[B_d, L]
                #only update designable positions, keep fixed positions as S_true
                S = (S_next*chain_mask.long() + S_true*(1 - chain_mask.long())).long()

                #record latest probs/log_probs at designable positions
                all_probs = (chain_mask[:,:,None]*probs_sample).float()
                all_log_probs = (chain_mask[:,:,None]*log_probs).float()

                t_value += h
                step_idx += 1
                if trajectory_writer is not None:
                    trajectory_writer.write_step(step_idx=step_idx, t=t_value, S=S, mask_token_id=mask_token_id, probs=probs_sample)

        decoding_order = torch.arange(L, device=device)[None,:].repeat(B_decoder, 1)
        output_dict = {"S": S, "sampling_probs": all_probs, "log_probs": all_log_probs, "decoding_order": decoding_order}
        return output_dict

    def sample_ar(self, feature_dict):
        #xyz_37 = feature_dict["xyz_37"] #[B,L,37,3] - xyz coordinates for all atoms if needed
        #xyz_37_m = feature_dict["xyz_37_m"] #[B,L,37] - mask for all coords
        #Y = feature_dict["Y"] #[B,L,num_context_atoms,3] - for ligandMPNN coords
        #Y_t = feature_dict["Y_t"] #[B,L,num_context_atoms] - element type
