...
def main(args):
    ...
    if args.model_mode not in ("ar", "dfm"):
        print("Must provide --model_mode as either 'ar' or 'dfm'")
        sys.exit()
    checkpoint_mode = args.model_mode
    model = ProteinMPNN(
        node_features=128,
        edge_features=128,
        hidden_dim=128,
    ...
    #load pretrained parameters — prefer EMA weights if available
    state_dict = checkpoint['model_state_dict']
    if 'ema_state_dict' in checkpoint:
        ema_weights = {k[len('ema_model.'):]: v
    ...
    # build job list: (pdb_path, fixed_residues, out_folder)
    if args.pdb_list_file:
        pdb_jobs = []
        with open(args.pdb_list_file) as _f:
            for _line in _f:
                _parts = _line.strip().split('\t')
                if len(_parts) >= 2:
                    pdb_jobs.append((_parts[0], [], _parts[1]))
    elif args.fixed_pos_by_pdb:
        with open(args.fixed_pos_by_pdb, 'r') as fh:
            fixed_pos_by_pdb = json.load(fh)
        pdb_jobs = [(pdb, fixed_res, args.out_folder) for pdb, fixed_res in fixed_pos_by_pdb.items()]
    else:
        _fixed = [item for item in args.fixed_residues.split()]
        pdb_jobs = [(args.pdb_path, _fixed, args.out_folder)]

    for pdb, fixed_residues, folder_for_outputs in pdb_jobs:
        ...
        try:
            #adjust input PDB name, stripping .gz then .pdb/.cif
            ...
            if name.endswith(".gz"):
                name = name[:-3]
                _tmp_file = tempfile.NamedTemporaryFile(suffix=os.path.splitext(name)[1], delete=False)
                with gzip.open(pdb, 'rb') as _gf:
                    shutil.copyfileobj(_gf, _tmp_file)
                _tmp_file.close()
                _pdb_for_parse = _tmp_file.name
            if name[-4:] == ".pdb":
                name = name[:-4]
            elif name[-4:] == ".cif":
                name = name[:-4]
            ...
            with torch.no_grad():
                ...
                feature_dict["dfm_dt"] = args.dfm_dt
                ...
                if checkpoint_mode == "ar":
                ...
                elif checkpoint_mode == "dfm":
                    if args.dfm_schedule == "eds":
                        if not args.eds_schedule_path:
                            print("--dfm_schedule eds requires --eds_schedule_path", file=sys.stderr); sys.exit(1)
                        from samplers import EDSSampler
                        with open(args.eds_schedule_path) as fh:
                            sched = json.load(fh)
                        def _make_sampler():
                            return EDSSampler(t_grid=sched["t_grid"])
                    else:
                        if args.dfm_dt is None:
                            print("--dfm_dt is required when --model_mode dfm", file=sys.stderr); sys.exit(1)
                        def _make_sampler():
                            return EulerSampler(dt=args.dfm_dt)
                    # Trajectory writer: one JSONL file per PDB. We append batch by batch
                    # (sample_id_offset accumulates) so all designs for a structure end up
                    # in one file.
                    traj_writer = None
                    sample_id_offset = 0
                    if args.trajectory_dir:
                        from trajectory import TrajectoryWriter
                        traj_path = os.path.join(args.trajectory_dir, f"{name}.jsonl")
                        # Truncate any prior content for this structure so reruns are clean.
                        os.makedirs(args.trajectory_dir, exist_ok=True)
                        open(traj_path, "w").close()
                    for _ in range(args.number_of_batches):
                        feature_dict["randn"] = torch.randn([feature_dict["batch_size"], feature_dict["mask"].shape[1]], device=device)
                        #main step-----
                        dfm_sampler = _make_sampler()
                        if args.trajectory_dir:
                            # Build chain_mask of shape [B_d, L] matching the broadcasted batch inside sample_dfm.
                            cm = feature_dict["chain_mask"] * feature_dict["mask"]
                            cm = cm.repeat(feature_dict["batch_size"], 1)
                            traj_writer = TrajectoryWriter(traj_path, structure_name=name,
                                                           sample_id_offset=sample_id_offset,
                                                           chain_mask=cm)
                        output_dict = model.sample(feature_dict, mode="dfm", sampler=dfm_sampler,
                                                   trajectory_writer=traj_writer)
                        if traj_writer is not None:
                            traj_writer.close()
                        sample_id_offset += feature_dict["batch_size"]
    
                        #compute confidence scores
                        combined_mask = feature_dict["mask"]*feature_dict["chain_mask"]
                        loss, loss_per_residue = get_score(output_dict["S"], output_dict["log_probs"], combined_mask, num_letters)
                        loss_XY, _ = get_score(output_dict["S"], output_dict["log_probs"], combined_mask, num_letters)
                        #-----
                        S_list.append(output_dict["S"])
                        log_probs_list.append(output_dict["log_probs"])
                    ...
                    print(f"Unknown model mode: {checkpoint_mode}. Choose from: ar, dfm")
                    sys.exit()
            ...
            import traceback
            if '_tmp_file' in dir() and _tmp_file is not None:
                try: os.unlink(_tmp_file.name)
                except: pass
            _fail_dir = os.path.join(base_folder, "failed_inferences")
            os.makedirs(_fail_dir, exist_ok=True)
            _fail_name = pdb[pdb.rfind("/")+1:]
            with open(os.path.join(_fail_dir, _fail_name + ".txt"), "w") as _fh:
                _fh.write(traceback.format_exc())
            print(f"  FAILED {_fail_name}: {_e}", file=sys.stderr)
            torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--pdb_list_file", type=str, default=None, help="TSV with pdb_path<TAB>out_folder per line. Processes all with one model load, ignoring --pdb_path.")
    ...
    argparser.add_argument("--dfm_dt", type=float, required=False, default=None, help="DFM step size (only used when --model_mode dfm).")
    argparser.add_argument("--dfm_sampler", type=str, default="euler", choices=["euler"], help="DFM sampler type (only used when --model_mode dfm and --dfm_schedule uniform).")
    argparser.add_argument("--dfm_schedule", type=str, default="uniform", choices=["uniform", "eds"], help="DFM time schedule: 'uniform' (uses --dfm_dt) or 'eds' (loads --eds_schedule_path).")
    argparser.add_argument("--eds_schedule_path", type=str, default=None, help="Path to JSON produced by build_eds_schedule.py (required when --dfm_schedule eds).")
    argparser.add_argument("--trajectory_dir", type=str, default=None, help="If set, write per-step DFM sampling trajectories as JSONL into this dir (one file per PDB).")
    argparser.add_argument("--model_mode", type=str, default=None, required=True, help="Inference mode: 'ar' for autoregressive, 'dfm' for discrete flow matching.")
