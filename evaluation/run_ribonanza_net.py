...
DEFAULT_RIBONANZANET_BASE = os.environ.get(
    "RIBONANZA_NET_BASE",
    "/rds/user/mh2167/hpc-work/NA-MPNN/evaluation/models/ribonanzanet"
)
if not os.path.exists(DEFAULT_RIBONANZANET_BASE):
    DEFAULT_RIBONANZANET_BASE = os.path.join(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
        "evaluation/models/ribonanzanet"
    )
sys.path.append(os.path.join(DEFAULT_RIBONANZANET_BASE, "kaggle/input/RibonanzaNet2D_Final"))
from Network import RibonanzaNet

# Setup ARNIE.
sys.path.append(os.path.join(DEFAULT_RIBONANZANET_BASE, "arnie", "src"))
...
def run_ribonanza_net_reactivity_profile(sequence,
    ...
    """    
    # Convert the sequence to a tensor.
    seq_tensor = ribonanza_net_seq_to_tensor(sequence).to(device)
    ...
    for i in range(batch_size):
        predicted_reactivity_profile = \
            model(seq_tensor, torch.ones_like(seq_tensor).detach().to(device))
        predicted_reactivity_profile = predicted_reactivity_profile.detach().cpu().numpy()
        predicted_2A3_reactivity_profiles.append(predicted_reactivity_profile[0, :, 0].tolist())
        predicted_DMS_reactivity_profiles.append(predicted_reactivity_profile[0, :, 1].tolist())
...
if __name__ == "__main__":
    ...
    config_path = os.environ.get(
        ...
        os.path.join(
            DEFAULT_RIBONANZANET_BASE,
            "kaggle/input/RibonanzaNet2D_Final/configs/pairwise.yaml",
        ),
    )
    if mode == "reactivity_profile":
        ...
        weights_path = os.environ.get(
            ...
            os.path.join(
                DEFAULT_RIBONANZANET_BASE,
                "kaggle/input/RibonanzaNet_Weights/RibonanzaNet.pt",
            ),
        )
        ...
        weights_path = os.environ.get(
            ...
            os.path.join(
                DEFAULT_RIBONANZANET_BASE,
                "kaggle/input/RibonanzaNet_Weights/RibonanzaNet-SS.pt",
            ),
        )
    else:
