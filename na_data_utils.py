...
class PDBDataset(torch.utils.data.Dataset):
    def __init__(self, 
                 ...
                 chain_cluster_lookup=None):
        self.chain_cluster_lookup = chain_cluster_lookup if chain_cluster_lookup is not None else {}
    ...
    def load_assembly(self, macromolecule_chain_dict, asmb, assembly_id, ppms, pdb_id=None):
        X_list = []
        protein_mask_list = []
        dna_mask_list = []
        rna_mask_list = []
        ...
        R_idx_list = []
        chain_labels_list = []
        chain_letter_labels_list = []
        # Per-residue CD-HIT sequence cluster ID; used by DFM to find
        # same-cluster chains in the assembly for independent masking.
        cluster_id_labels_list = []
        chain_multi = 0

        unique_letters = list(dict.fromkeys(
            letter for letter, _ in asmb[assembly_id] if letter in macromolecule_chain_dict
        ))
        letter_to_id = {l: i for i, l in enumerate(unique_letters)}
        ...
        for i, (letter, transform_matrix) in enumerate(asmb[assembly_id]):
            if letter in macromolecule_chain_dict:
                ...
                # Shared ID for all copies of the same chain letter in this assembly
                # (e.g. both copies of chain 'A' in a homodimer get the same ID).
                # Used by DFM to synchronize masking across homologous chains.
                chain_letter_labels_list.append(letter_to_id[letter]*np.ones_like(macromolecule_chain_dict[letter]["residue_idx"], dtype=np.int32))
                # Look up CD-HIT cluster ID for this chain (-1 if not found, e.g. protein chains).
                cluster_id = self.chain_cluster_lookup.get(pdb_id, {}).get(letter, -1) if pdb_id else -1
                cluster_id_labels_list.append(cluster_id*np.ones_like(macromolecule_chain_dict[letter]["residue_idx"], dtype=np.int32))
                chain_multi += 1
        ...
        chain_letter_labels = np.concatenate(chain_letter_labels_list, axis = 0) #[L]
        cluster_id_labels = np.concatenate(cluster_id_labels_list, axis = 0) #[L]
        ...
        out_dict["chain_letter_labels"] = chain_letter_labels[mask_for_output]
        out_dict["cluster_id_labels"] = cluster_id_labels[mask_for_output]
    ...
    def loader(self, example_dict, assembly_id):
        ...
        # Extract pdb_id from structure path for cluster lookup (e.g. "100d" from "100d.cif.gz").
        structure_name = os.path.basename(example_dict["structure_path"])
        pdb_id = structure_name.split('.')[0]

        out_dict = self.load_assembly(macromolecule_chain_dict, asmb, assembly_id, ppms, pdb_id=pdb_id)
