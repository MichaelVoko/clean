...
# Helper functions used in the training loop
def featurize(batch, polytype_to_int, restype_to_int, atom_dict, device):
    ...
    if B > 0:
        ...
        chain_letter_labels = -1*torch.ones([B, L_max], dtype=torch.int64)
        cluster_id_labels = -1*torch.ones([B, L_max], dtype=torch.int64)
        ...
        for i, b in enumerate(batch):
            ...
            chain_letter_labels[i,:L_stack[i]] = out_dict["chain_letter_labels"][None,]
            cluster_id_labels[i,:L_stack[i]] = out_dict["cluster_id_labels"][None,]
        ...
        out_dict["chain_letter_labels"] = chain_letter_labels.to(device) #[B, L] shared ID across exact-match chains in homomers
        out_dict["cluster_id_labels"] = cluster_id_labels.to(device) #[B, L] CD-HIT cluster ID per residue (-1 for protein/unknown)
...
def loss_smoothed(S,
    ...
    denom = torch.sum(mask).clamp_min(1) if mean_over_mask else tokens
    loss_av = torch.sum(loss * mask) / denom
    return loss, loss_av
...
"""
These classes are the main model components: the encoder and decoder layers, the protein feature extraction module and the overall MPNN model.
The encoder layer updates both node and edge features with parallel attention, while the decoder layer only updates node features with masked attention.
The feature extraction module extracts node and edge features from the input data, including distance-based features and positional encodings. 
The overall MPNN model combines these components to perform masked sequence modelling on protein and nucleic acid structures.
"""
class EncLayer(nn.Module): # Standard transformer layer with parallel node and edge updates
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        super(EncLayer, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale # normalization factor for the aggregated messages, can be tuned as a hyperparameter
        ...
        self.adaLN_modulation = nn.Sequential(nn.SiLU(), nn.Linear(num_hidden, 2 * num_hidden, bias=True))
        nn.init.zeros_(self.adaLN_modulation[-1].weight)
        nn.init.zeros_(self.adaLN_modulation[-1].bias)

    def forward(self, h_V, h_E, E_idx, mask_V=None, mask_attend=None, time=None):
...
class DecLayer(nn.Module): # updates node features with partially masked attention and no edge updates
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
...
class PositionWiseFeedForward(nn.Module): # Just a simple two-layer MLP with activation in between, applied to each position separately and identically
...
class PositionalEncodings(nn.Module): # Used in protein features 
...
class ProteinMPNN(nn.Module):
    def __init__(self, 
        ...
        if mode == "dfm":
            self.W_t = TimestepEmbedder(hidden_dim)
    ...
    def forward_ar(self, feature_dict):
        X = feature_dict["X"] 
        S = feature_dict["S"]
        mask = feature_dict["mask"]
        protein_mask = feature_dict["protein_mask"]
        ...
        h_EXV_encoder = mask_fw * h_EXV_encoder
        for layer in self.decoder_layers:
            h_ESV = cat_neighbors_nodes(h_V, h_ES, E_idx)
            h_ESV = mask_bw * h_ESV + h_EXV_encoder
            if self.training:
    ...
    def forward_dfm(self, feature_dict, t):
        X = feature_dict["X"] 
        S = feature_dict["S"]
        mask = feature_dict["mask"]
        ...
        # Scale t to [0, 1000] so sinusoidal frequencies span their full range
        t = t.to(h_V.device).float().view(t.shape[0])  # [B]
        t_embed = self.W_t(t * 1000.0).unsqueeze(1)  # [B, 1, H]

        # Encoder — t injected at the start of every layer
        mask_attend = gather_nodes(mask.unsqueeze(-1),  E_idx).squeeze(-1)
        mask_attend = mask.unsqueeze(-1) * mask_attend
        ...
        for layer in self.encoder_layers:
            if self.training:
                h_V, h_E = torch.utils.checkpoint.checkpoint(
                    layer, h_V, h_E, E_idx, mask, mask_attend, t_embed, use_reentrant=False)
            else:
                h_V, h_E = layer(h_V, h_E, E_idx, mask, mask_attend, t_embed)

        h_S = self.W_s(S)
        h_ES = cat_neighbors_nodes(h_S, h_E, E_idx)

        # Decoder — t injected at the start of every layer
        for layer in self.decoder_layers:
            h_ESV = cat_neighbors_nodes(h_V, h_ES, E_idx)
            if self.training:
                h_V = torch.utils.checkpoint.checkpoint(
                    layer, h_V, h_ESV, mask, mask_attend, t_embed, use_reentrant=False)
            else:
                h_V = layer(h_V, h_ESV, mask, mask_attend, t_embed)
    ...
    def forward(self, feature_dict, mode = "ar", t=None):
        if mode == "ar":
            return self.forward_ar(feature_dict)
        elif mode == "dfm":
            if t is None:
                raise Exception("Time step t needed for DFM forward")
            return self.forward_dfm(feature_dict, t)
        else:
            raise Exception("Invalid forward mode")
